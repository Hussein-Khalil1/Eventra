...
export const purchaseTicket = mutation({
  ...
  handler: async (
    ctx,
    { eventId, userId, waitingListId, quantity, attendeeNames, paymentInfo }
  ) => {
    ...
    const existingTickets = paymentInfo.paymentIntentId
      ? await ctx.db
          .query("tickets")
          .withIndex("by_payment_intent", (q) =>
            q.eq("paymentIntentId", paymentInfo.paymentIntentId)
          )
          .collect()
      : [];
    const alreadyCreated = existingTickets.length;
    const safeQuantity = Number.isFinite(quantity) ? Math.floor(quantity) : 1;
    const requestedQuantity = Math.max(1, safeQuantity);
    const remainingToCreate = Math.max(0, requestedQuantity - alreadyCreated);

    if (remainingToCreate === 0) {
      console.log("Tickets already exist for payment intent");
      if (waitingListEntry.status !== WAITING_LIST_STATUS.PURCHASED) {
    ...
    if (
      waitingListEntry.status !== WAITING_LIST_STATUS.OFFERED &&
      waitingListEntry.status !== WAITING_LIST_STATUS.PURCHASED &&
      waitingListEntry.status !== WAITING_LIST_STATUS.EXPIRED
    ) {
    ...
    if (waitingListEntry.status === WAITING_LIST_STATUS.EXPIRED) {
      console.warn("Proceeding with purchase for expired offer", {
        waitingListId,
      });
    }
    ...
    try {
      ...
      const hasActiveOffer =
        waitingListEntry.status === WAITING_LIST_STATUS.OFFERED &&
        (waitingListEntry.offerExpiresAt ?? 0) > now;
      const activeOffersExcludingCurrent = Math.max(
        0,
        activeOffers - (hasActiveOffer ? 1 : 0)
      );

      const availableToSell = Math.max(
        0,
        event.totalTickets - (purchasedCount + activeOffersExcludingCurrent)
      );

      if (remainingToCreate > availableToSell) {
        console.error("Not enough tickets available to fulfill purchase", {
          remainingToCreate,
          availableToSell,
        });
        throw new Error("Not enough tickets available to fulfill purchase");
      }

      const normalizedAttendeeNames = (attendeeNames ?? []).map((name) =>
        name.trim()
      );
      console.log("Creating ticket with payment info", paymentInfo);
      const baseTime = Date.now();
      for (let i = 0; i < remainingToCreate; i += 1) {
        const attendeeName = normalizedAttendeeNames[alreadyCreated + i];
        await ctx.db.insert("tickets", {
          eventId,
          userId,
          purchasedAt: baseTime,
          status: TICKET_STATUS.VALID,
          paymentIntentId: paymentInfo.paymentIntentId,
          amount: paymentInfo.amount,
          attendeeName: attendeeName ? attendeeName : undefined,
        });
      }
      ...
      // Process queue for next person (scheduled immediately)
