...
export default function Ticket({ ticketId }: { ticketId: Id<"tickets"> }) {
  ...
  const ticketLoading = ticket === undefined;
  ...
  if (!ticket || !ticket.event || !user) {
    return (
      <div className="bg-white rounded-xl shadow-xl border border-gray-100 p-6 text-center text-gray-600">
        Ticket not found.
      </div>
    );
  }

  const attendeeName = ticket.attendeeName?.trim();
  const displayName = attendeeName || user.name;
  const showPurchaser = Boolean(attendeeName && attendeeName !== user.name);
  ...
  return (
    ...
    >
        ...
        {imageUrl && (
          <div className="relative w-full aspect-[21/9] overflow-hidden">
            <Image
              ...
              alt=""
              fill
              className={`object-cover object-center blur-[36px] scale-[1.25] ${ticket.event.is_cancelled ? "opacity-100" : ""}`}
              aria-hidden="true"
            />
            ...
            <Image
              ...
              fill
              className={`object-contain object-center z-20 ${ticket.event.is_cancelled ? "opacity-50" : ""}`}
              priority
            />
            <div className="absolute inset-0 bg-gradient-to-b from-black/50 to-black/90 z-30 pointer-events-none" />
          </div>
        )}
        <div
          className={`px-6 py-4 ${imageUrl ? "absolute bottom-0 left-0 right-0" : ticket.event.is_cancelled ? "bg-red-600" : "bg-yellow-300"} `}
        >
          <h2
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center text-gray-600">
              <CalendarDays
                className={`w-5 h-5 mr-3 ${ticket.event.is_cancelled ? "text-red-600" : "text-yellow-400"}`}
              />
              <div>
      ...
      >
        ...
        <span
          className={`text-sm font-medium ${ticket.event.is_cancelled ? "text-red-600" : "text-yellow-400"}`}
        >
